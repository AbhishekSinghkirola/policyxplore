<?php
    ...
    if(isset($_GET['id'])) {
        ...
        $res = mysqli_query($con,"select * from faq_page where id = $id");
        if(mysqli_num_rows($res) > 0) {
            $row = mysqli_fetch_assoc($res);
        }else {
           echo "<script>window.location.href = 'faq-page.php'; </script>";
        }

    }else  {
        header('loction: faq-page.php');
    }

    if(isset($_POST['submit'])) {
        $faqp_question=get_safe_value($con,$_POST['faqp_question']);
        $faqp_solution=get_safe_value($con,$_POST['faqp_solution']);
        $faq_category=get_safe_value($con,$_POST['faq_category']);

        
        $old_faq_category = get_safe_value($con,$_POST['old_faq_category']);
        
        mysqli_query($con,"update faq_page set faqp_question='$faqp_question',faqp_solution='$faqp_solution',faq_category='$faq_category' where id = $id") or die("Update Qquery Failed!!");
            ...
            <div class="card-body">
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="basic-default-name" name="faqp_question" value="<?php echo $row['faqp_question']; ?>" />
                        </div>
                    </div>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <textarea id="basic-default-message" class="form-control" aria-label="Hi, Do you have a moment to talk Joe?" aria-describedby="basic-icon-default-message2" name="faqp_solution" required><?php echo $row['faqp_    solution']; ?></textarea>
                        </div>
                    </div>
